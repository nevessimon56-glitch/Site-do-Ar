(function () {
  ...
  var state = {
    step: 0,
    answers: {},
    scores: { economy: 0, comfort: 0, speed: 0, silence: 0, tech: 0, power: 0 },
    btuRaw: 0,
    btuBase: 0,
    exceedsCatalog: false,
    rangeInfo: null
  };

  var CATALOG_MAX_BTU = 56000;
  ...
  /** BTU estimado só após informar área (pergunta roomSize). */
  function hasBtuInputs() {
    return state.answers.roomSize !== undefined && state.answers.roomSize !== null;
  }

  /* Converte respostas do quiz nos parâmetros da calculadora BTUs (assets/calculadora-btus-wizard.js) */
  function mapAnswersToBtuParams() {
    ...
    if (state.answers.roomType !== undefined) {
      var solByType = [1, 1.1, 1.25, 1, 1];
      fatSol = solByType[state.answers.roomType] || 1;
      if (state.answers.roomType === 3) fatPe = 3 / 2.6;
      if (state.answers.roomType === 4) fatLayout = 1.15;
    }

    if (state.answers.windows !== undefined) {
      var janelasByAns = [1, 1.08, 1.18, 1.18];
      fatJanelas = janelasByAns[state.answers.windows] || 1.08;
    }

    if (state.answers.floor !== undefined) {
      var andarByAns = [1.05, 1, 1.15];
      fatAndar = andarByAns[state.answers.floor] || 1;
    }

    if (state.answers.insulation !== undefined) {
      var isolByAns = [0.95, 1, 1.12];
      fatIsol = isolByAns[state.answers.insulation] || 1;
    }

    var fatUso = 1.1;
    var equip = 0;
    if (state.answers.environment !== undefined) {
      var usoByEnv = [1, 1.1, 1.1, 1.5, 1.2, 1.2];
      var equipByEnv = [0, 600, 1500, 3000, 2500, 800];
      fatUso = usoByEnv[state.answers.environment] || 1.1;
    ...
    if (state.answers.usage !== undefined) {
      var horasByUsage = [1, 1.05, 1.1, 1.15];
      fatHoras = horasByUsage[state.answers.usage] || 1;
    ...
    if (state.answers.climate !== undefined) {
      var climaByAns = [1.15, 1.08, 1, 1];
      fatClima = climaByAns[state.answers.climate] || 1;
    }

    if (state.answers.humidity !== undefined) {
      var umidByAns = [1, 1, 1.05];
      fatUmid = umidByAns[state.answers.humidity] || 1;
    }
  ...
  function recommendBtuRange(raw) {
    if (raw > CATALOG_MAX_BTU) {
      return {
        ...
        url: null,
        raw: raw,
        catalogMax: CATALOG_MAX_BTU
      };
    }
    for (var i = 0; i < BTU_RANGES.length; i++) {
      if (raw <= BTU_RANGES[i].max) {
        return {
          exceedsCatalog: false,
          btu: BTU_RANGES[i].btu,
          label: BTU_RANGES[i].label,
          url: BTU_RANGES[i].url,
          raw: raw
        };
      }
    }
    var last = BTU_RANGES[BTU_RANGES.length - 1];
    return {
      exceedsCatalog: true,
      btu: null,
      label: last.label,
  ...
  function recommendBtu(raw) {
    var info = recommendBtuRange(raw);
    return info.exceedsCatalog ? null : info.btu;
  }
  ...
  function recalc() {
    ...
    if (hasBtuInputs()) {
      ...
      state.rangeInfo = recommendBtuRange(state.btuRaw);
      state.exceedsCatalog = state.rangeInfo.exceedsCatalog;
      state.btuBase = state.exceedsCatalog ? 0 : state.rangeInfo.btu;
    } else {
      state.btuRaw = 0;
      state.btuBase = 0;
      state.exceedsCatalog = false;
      state.rangeInfo = null;
    }
    updateUI();
  }
  ...
  function updateUI() {
    ...
    var canBtu = hasBtuInputs();
    var btu = state.btuBase;
    var btuHtml;
    var btuText;
    if (!canBtu) {
      btuHtml = '—<span class="d-btu-unit"> BTUs/h</span>';
      btuText = 'Responda à área do ambiente';
    } else if (state.exceedsCatalog) {
      btuHtml =
        fmtBtuNum(state.btuRaw) +
        '<span class="d-btu-unit"> BTUs/h</span> <span class="d-btu-warn">(acima do catálogo)</span>';
      btuText = fmtBtuNum(state.btuRaw) + ' BTUs/h — acima da faixa disponível';
    } else {
      btuHtml = fmtBtuHtml(btu);
      btuText = fmtBtuNum(btu) + ' BTUs/h (estimativa)';
    }
    var status = !canBtu
    ...
    if (canBtu && state.exceedsCatalog) {
      els.heroStatus.textContent = 'Estimativa acima do catálogo — consulte um especialista';
    }
  ...
  function buildWa(persona, btu, priority, feats) {
    ...
    var btuLine = state.exceedsCatalog
      ? '🌡 *Carga estimada:* ' +
        fmtBtuNum(state.btuRaw) +
        ' BTUs/h (acima do catálogo online — preciso de dimensionamento profissional)'
      : '🌡 *BTUs sugeridos:* ' + fmtBtuNum(btu) + ' BTUs/h (estimativa preliminar)';
    var lines = [
      'Ola! Fiz o Diagnostico Clima 360 no site.',
      '',
      '📊 *Meu perfil:* ' + persona.title,
      btuLine,
      '⚡ *Prioridade:* ' + priority,
      ''
    ];
  ...
  function buildLink(rawBtu) {
    ...
    if (range.exceedsCatalog) return null;
    return 'https://www.sitedoar.com.br' + range.url + '?pg=1';
  }

  function buildConsultWa(persona, priority, feats) {
    return buildWa(persona, state.exceedsCatalog ? state.btuRaw : state.btuBase, priority, feats);
  }
  ...
  function buildResult() {
    ...
    var btu = state.btuBase;
    var exceeds = state.exceedsCatalog;
    ...
    var primaryCta = document.getElementById('dPrimaryCta');
    var waCta = document.getElementById('dWaCta');
    var routeDisc = document.querySelector('.d-route-disc');

    document.getElementById('dResultTitle').textContent = exceeds
      ? 'Dimensionamento acima do catálogo online'
      : p.title;

    if (exceeds) {
      document.getElementById('dResultText').textContent =
        'Estimativa preliminar: ' +
        fmtBtuNum(state.btuRaw) +
        ' BTUs/h. Isso ultrapassa a maior capacidade disponível na loja (' +
        fmtBtuNum(CATALOG_MAX_BTU) +
        ' BTUs/h). Não recomendamos comprar um aparelho menor. ' +
        p.text;
    } else {
      document.getElementById('dResultText').textContent =
        p.text +
        ' Faixa sugerida: ' +
        fmtBtuNum(btu) +
        ' BTUs/h (cálculo bruto: ' +
        fmtBtuNum(state.btuRaw) +
        ' BTUs/h). Estimativa preliminar — confirme com avaliação técnica em ambientes exigentes.';
    }

    document.getElementById('dRouteTitle').textContent = exceeds
      ? 'Próximo passo: projeto / avaliação técnica'
      : hasUrgency
        ? 'Rota: atendimento imediato'
        : 'Rota recomendada';
    document.getElementById('dRouteText').textContent = exceeds
      ? 'Fale com nosso time para dimensionamento profissional (possível solução multi-aparelho ou linha comercial).'
      : p.route + (feats.length ? ' Recursos citados: ' + feats.join(', ') + '.' : '');

    if (routeDisc) {
      routeDisc.textContent = exceeds
        ? 'Resultado orientativo. Carga acima do catálogo exige validação de um profissional de climatização.'
        : 'As sugestões são estimativas com base nas respostas. Para ambientes complexos ou comerciais, recomendamos atendimento especializado.';
    }

    var productUrl = buildLink(state.btuRaw);
    if (primaryCta) {
      if (exceeds || !productUrl) {
        primaryCta.href = buildConsultWa(p, dims[topKey], feats);
        primaryCta.textContent = '💬 Dimensionamento com especialista';
        primaryCta.classList.add('d-btn-consult');
      } else {
        primaryCta.href = productUrl;
        primaryCta.textContent = '🛒 Ver produtos indicados';
        primaryCta.classList.remove('d-btn-consult');
      }
    }
    if (waCta) waCta.href = buildConsultWa(p, dims[topKey], feats);

    document.getElementById('dResultBadges').innerHTML = [
      exceeds
        ? { label: fmtBtuNum(state.btuRaw) + ' BTUs/h (bruto)', h: true }
        : { label: fmtBtuNum(btu) + ' BTUs/h', h: true },
      { label: dims[topKey], h: true },
      exceeds
        ? { label: 'Acima do catálogo', h: false }
        : { label: hasUrgency ? 'Alta urgência' : 'Comparar opções', h: false }
    ]
      .concat(feats.slice(0, 2).map(function (l) { return { label: l, h: false }; }))
      .map(function (b) {
        return '<span class="d-badge' + (b.h ? ' hl' : '') + '">' + b.label + '</span>';
      })
      .join('');

...
(function () {
  ...
  function refreshThemeShowcaseSliders(force) {
    ...
    for (var i = 0; i < lists.length; i++) {
      if (mobileViewport) {
        if (lists[i].classList.contains('showcase-list--grid-mobile-2col')) continue;
        if (lists[i].closest && (lists[i].closest('.product-showcase') || lists[i].closest('.col-product-related') || lists[i].closest('.col-product-visited'))) continue;
      }
    ...
    if (typeof jQuery !== 'undefined' && jQuery.fn && jQuery.fn.slick) {
      jQuery('.showcase-list.slick-initialized').each(function () {
        if (mobileViewport && this.closest && (this.closest('.product-showcase') || this.closest('.col-product-related') || this.closest('.col-product-visited'))) {
          return;
        }
        try {

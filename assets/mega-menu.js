...
(function () {
  ...
  function normalizeOrderActionText(value) {
    return (value || '').replace(/\s+/g, ' ').trim().toLowerCase();
  }

  function extractOrderIdFromMenu(menu, dropdown) {
    if (!menu) return null;
    ...
    for (i = 0; i < links.length; i++) {
      ...
      match = href.match(/\/pedidos\/(\d+)(?:[/?#]|$)/i);
      if (match) return match[1];

      match = href.match(/[?&](?:id|pedido|orderId)=(\d+)/i);
      if (match) return match[1];
    }

    var scope = dropdown ? dropdown.closest('.page-accordion-info, .page-table-item, tr, .accordion-body-item') : null;
    if (scope) {
      var viewLink = scope.querySelector('a[href*="/pedidos/"]');
      if (viewLink) {
        match = (viewLink.getAttribute('href') || '').match(/\/pedidos\/(\d+)/i);
        if (match) return match[1];
      }
    }
  ...
  function menuHasCancelAction(menu) {
    ...
    for (i = 0; i < links.length; i++) {
      ...
      text = normalizeOrderActionText(links[i].textContent || links[i].getAttribute('title'));

      if (href.indexOf('cancel') !== -1) return true;
      if (text.indexOf('cancel') !== -1) return true;
    }

    return false;
  }

  function createCancelMenuItem(orderId, templateLink) {
    var item = document.createElement('li');
    var link = document.createElement('a');
    var className = 'menu-item';

    link.href = '/pedidos/cancelamento?id=' + encodeURIComponent(orderId);
    link.title = 'Solicitar cancelamento';
    link.textContent = 'Solicitar cancelamento';

    if (templateLink && templateLink.className) {
      link.className = templateLink.className.replace(/\s*menu-item-link_disabled\b/g, '').trim();
      className = templateLink.parentElement ? templateLink.parentElement.className || className : className;
    }
  ...
  function enableDisabledCancelLink(link, orderId) {
    link.classList.remove('menu-item-link_disabled');
    link.removeAttribute('aria-disabled');
    link.href = '/pedidos/cancelamento?id=' + encodeURIComponent(orderId);
    link.title = link.title || 'Solicitar cancelamento';

    var textEl = link.querySelector('.menu-item-link_text');
    if (textEl && normalizeOrderActionText(textEl.textContent).indexOf('cancel') === -1) {
      textEl.textContent = 'Solicitar cancelamento';
    } else if (normalizeOrderActionText(link.textContent).indexOf('cancel') === -1) {
      link.textContent = 'Solicitar cancelamento';
    }

    var icon = link.querySelector('.menu-item-disabled_icon');
    if (icon) icon.parentNode.removeChild(icon);
  }

  function injectOrdersCancelAction(root) {
    if (!/\/pedidos\/?$/i.test(window.location.pathname)) return;

    var scope = root && root.querySelector ? root : document;
    var dropdowns = scope.querySelectorAll('.orders-main .dropdown');
    var d;
    ...
    for (d = 0; d < dropdowns.length; d++) {
      ...
      var i;
      var enabled = false;
      var garantiaItem = null;
      var items;
      var cancelItem;

      if (!menu || menu.getAttribute('data-ar-cancel-injected') === '1') continue;

      orderId = extractOrderIdFromMenu(menu, dropdown);
      if (!orderId) continue;

      links = menu.querySelectorAll('a');
      for (i = 0; i < links.length; i++) {
        var href = (links[i].getAttribute('href') || '').toLowerCase();
        var text = normalizeOrderActionText(links[i].textContent || links[i].getAttribute('title'));

        if (href.indexOf('cancel') !== -1 || text.indexOf('cancel') !== -1) {
          if (
            links[i].classList.contains('menu-item-link_disabled') ||
            !links[i].getAttribute('href') ||
            links[i].getAttribute('href') === '#'
          ) {
            enableDisabledCancelLink(links[i], orderId);
          }
          enabled = true;
          break;
        }
      ...
      if (!enabled && !menuHasCancelAction(menu)) {
        items = menu.querySelectorAll('.menu-item');
        for (i = 0; i < items.length; i++) {
          if (normalizeOrderActionText(items[i].textContent).indexOf('garantia') !== -1) {
            garantiaItem = items[i];
            break;
          }
        }

        cancelItem = createCancelMenuItem(orderId, menu.querySelector('.menu-item > a'));
        if (garantiaItem && garantiaItem.parentNode) {
          garantiaItem.parentNode.insertBefore(cancelItem, garantiaItem);
        } else {
          menu.appendChild(cancelItem);
        }
      }

      menu.setAttribute('data-ar-cancel-injected', '1');
    }
  }

  function initOrdersCancelAction() {
    if (!/\/pedidos\/?$/i.test(window.location.pathname)) return;

    function run() {
      injectOrdersCancelAction(document);
    }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initOrdersCancelAction);
  } else {
    initOrdersCancelAction();
  }
})();

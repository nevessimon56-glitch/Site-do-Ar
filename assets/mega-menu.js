...
(function () {
  ...
  function isTnsProductCarouselList(node) {
    if (!node || !node.closest) return false;
    if (node.closest('.showcase-products_carousel')) return true;
    if (node.classList && node.classList.contains('showcase-slider')) return true;
    if (node.closest('.tns-outer') && node.closest('main.home-main')) return true;
    return false;
  }

  function isProtectedShowcaseList(node) {
    if (!node || !node.closest) return false;
    if (isTnsProductCarouselList(node)) return true;
    if (node.closest('.product-showcase')) return true;
    if (node.classList && node.classList.contains('slick-initialized')) return true;
    if (node.closest('.slick-initialized')) return true;
    return false;
  }
  ...
  function refreshThemeShowcaseSliders(force) {
    ...
    for (var i = 0; i < lists.length; i++) {
      if (isProtectedShowcaseList(lists[i])) {
        continue;
      }
    ...
    if (typeof jQuery !== 'undefined' && jQuery.fn && jQuery.fn.slick) {
      jQuery('.showcase-list.slick-initialized').each(function () {
        if (isProtectedShowcaseList(this)) {
          return;
        }
        try {
  ...
  function bootstrapProductShowcase() {
    var root = document.querySelector('.product-showcase');
    if (!root) return;
    refreshShowcaseCards(root);
    if (typeof jQuery !== 'undefined' && jQuery.fn && jQuery.fn.slick) {
      jQuery(root).find('.showcase-list.slick-initialized').each(function () {
        try {
          jQuery(this).slick('setPosition');
        } catch (e) {}
      });
  ...
  window.bootstrapProductShowcase = bootstrapProductShowcase;
  ...
  window.addEventListener('load', function () {
    bootstrapProductShowcase();
    window.setTimeout(bootstrapProductShowcase, 600);
    window.setTimeout(bootstrapProductShowcase, 2000);
  });
...
(function () {
  ...
  function isCatalogPage() {
    if (document.querySelector('.showcase-search, main.search-main, .search-main')) return true;
  ...
  function unslickCatalogList(list) {
  ...
  function clearMobileGridInlineStyles(list) {
    if (!list) return;
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      items[i].style.removeProperty('float');
      items[i].style.removeProperty('display');
    }
  }

  function applyCatalogGrid(section) {
    if (!section) return;
    ...
    if (list) {
      unslickCatalogList(list);
      clearMobileGridInlineStyles(list);
    }
  }
  ...
  window.matchMedia(MOBILE_MQ).addEventListener('change', initCatalogMobileGrid);
  window.setTimeout(initCatalogMobileGrid, 400);
  window.setTimeout(initCatalogMobileGrid, 1500);
})();
...
/* ==========================================================
   HOME MOBILE — grade 2 col só na Promoção (.showcase-products_grid)
   Mais Vendidos (.showcase-products_carousel) = carrossel tns — NÃO alterar
...
(function () {
  ...
  function getTopNavItems(navMain) {
    var items = [];
    var child = navMain.firstElementChild;
    while (child) {
      if (child.classList && child.classList.contains('nav-main_item')) {
        items.push(child);
      }
  ...
  function shouldKeepNavItem(item) {
    ...
    if (text.indexOf('todos os produtos') !== -1 || path.indexOf('todos-os-produtos') !== -1) {
      return false;
    }
  ...
  function applyMobileNavStrip() {
    ...
    var items = getTopNavItems(navMain);
    var visible = 0;
...
/* NAV-FIXED-v15 — desktop: fixa nav | mobile: revela 3 cats só após rolar */
window.initSdaNavFixedTop = window.initSdaNavFixedTop || function initSdaNavFixedTop() {
  ...
  function update() {
    if (window.matchMedia(MOBILE_MQ).matches) {
      ...
      if (typeof window.initSdaMobileNavStrip === 'function') {
        window.initSdaMobileNavStrip();
      }

      if (readScrollY() >= getMobileRevealAt() - 1) {
        nav.classList.add('sda-mobile-nav-revealed');
        nav.classList.add('sda-mobile-nav-strip--3');
      } else {
        nav.classList.remove('sda-mobile-nav-revealed');
      }
      return;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', window.initSdaNavFixedTop);
} else {
  window.initSdaNavFixedTop();
}

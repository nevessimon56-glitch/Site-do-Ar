 ...
 * Fix: erros por etapa (sda-error-step1/2/3); não depende de #sda-error só na etapa 3.
 */
(function () {
  'use strict';

  var root = document.getElementById('sda-calculadora');
  ...
  function showStep(n) {
    ...
    if (txt) txt.textContent = 'Etapa ' + n + ' de 3';
    hideAllErrors();
  ...
  function calculate() {
    ...
    var pessoas = Math.max(1, parseInt(val('sda-pessoas') || '1', 10));
    var equip = parseInt(val('sda-equip') || '0', 10);
    var area = comp * larg;
    var p = {
      sol: val('sda-sol'),
      uso: val('sda-uso'),
    ...
    var idx = -1;
    for (var ii = 0; ii < catalog.length; ii++) {
    ...
    if (breakdown) {
      breakdown.innerHTML =
        keys
          ...
          .map(function (x) {
            var label =
              x[0] === 'pe' ? 'Pé-direito acima de 3 m' : labels[x[0]][p[x[0]]] || p[x[0]];
            return (
  ...
  var nextButtons = root.querySelectorAll('[data-next]');
  for (var ni = 0; ni < nextButtons.length; ni++) {
    nextButtons[ni].addEventListener('click', function () {
      if (!validate(step)) return;
      showStep(Math.min(3, step + 1));
    });
  }

  var prevButtons = root.querySelectorAll('[data-prev]');
  for (var pi = 0; pi < prevButtons.length; pi++) {
    prevButtons[pi].addEventListener('click', function () {
      showStep(Math.max(1, step - 1));
    });
  }

  var formEl = get('sda-form');
  if (formEl) {
  ...
  var calcBtn = root.querySelector('[data-calc-submit]');
  if (calcBtn) {
  ...
  var allFields = root.querySelectorAll('input,select');
  for (var fi = 0; fi < allFields.length; fi++) {
    allFields[fi].addEventListener('input', function () {
      if (!this.value) return;
      var parent = this.parentNode;
      while (parent && !(' ' + parent.className + ' ').match(/ field /)) {
